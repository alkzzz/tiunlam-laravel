...
{{ Form::open(array('route' => 'login', 'class'=>'form-signin')) }}
  ...
  <div class="form-group @if ($errors->has('password')) has-error @endif">
        ...
        <p>{{ Form::submit(trans('menu.sign_in'), array('class'=>'btn btn-lg btn-primary btn-block')) }}</p>
        <a class="btn btn-lg btn-success btn-block" href="{{ URL::route('homepage') }}">{{ trans('menu.back_to_home') }}</a>
        {{ Form::close() }}
      </form>
    </div> <!-- /container -->
